...
function Landing({ submit, error, loading }) {
  const [form, setForm] = useState(initialForm); 
  const [mode, setMode] = useState('create');
  const change = (key) => (e) => setForm({ ...form, [key]: e.target.value });
  const go = (e) => { e.preventDefault(); localStorage.setItem('watchwave-name', form.name.trim()); submit(mode, form); };
  return <main className="landing"><section className="brand"><div className="logo">W</div><span>watchwave</span></section><div className="hero-copy"><p className="eyebrow">SYNCED STREAMING, HUMAN MOMENTS</p><h1>Your people.<br /><i>One</i> timeline.</h1><p className="lede">Start a private YouTube room and watch together from anywhere. Every play, pause and laugh stays in sync.</p><div className="features"><span>◉ Live sync</span><span>◌ Private rooms</span><span>✦ No account needed</span></div></div><form className="join-card" onSubmit={go}><div className="segmented"><button type="button" className={mode === 'create' ? 'active' : ''} onClick={() => setMode('create')}>Create a party</button><button type="button" className={mode === 'join' ? 'active' : ''} onClick={() => setMode('join')}>Join a party</button></div><h2>{mode === 'create' ? 'Bring the crew together.' : 'You’re invited.'}</h2><p>{mode === 'create' ? 'Set up your private room in seconds.' : 'Enter the code shared by your host.'}</p><label>DISPLAY NAME<input value={form.name} onChange={change('name')} placeholder="e.g. Alex" maxLength="28" required /></label>{mode === 'create' ? <label>PARTY NAME<input value={form.title} onChange={change('title')} placeholder="Friday movie night" maxLength="60" /></label> : <label>ROOM CODE<input value={form.code} onChange={change('code')} placeholder="ABC123" maxLength="6" required /></label>}{error && <div className="form-error">{error}</div>}<button className="primary" disabled={loading}>{loading ? 'Connecting…' : mode === 'create' ? 'Create watch party →' : 'Join watch party →'}</button></form><footer>Built for the group chat that never sleeps.</footer></main>;
}

function Room({ room, selfId, notify, leave }) {
